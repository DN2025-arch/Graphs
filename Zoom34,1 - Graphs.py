...
##            visited=set()
    ...
    def largest_connected_components(self):
        ...
        for e in range(self.n):
            if not visited[e]:
                temp = []
                self.DFSUtil(e, temp, visited)
                if len(temp) > len(most_components):
                    most_components = temp
        return most_components #returns list, not length
...
g3.create_link(2,3)
g3.create_link(4,5)
print(len(g3.largest_connected_components()))
